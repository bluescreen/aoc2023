...
class Brick {
  ...
  public maxZ: number;
  constructor(constr: string) {
    [this.corner1, this.corner2] = constr
      .split("~")
      .map((p) => new Position(p));
    [this.minX, this.maxX] = [this.corner1.x, this.corner2.x].sort(
      (a, b) => a - b
    );
    [this.minY, this.maxY] = [this.corner1.y, this.corner2.y].sort(
      (a, b) => a - b
    );
    [this.minZ, this.maxZ] = [this.corner1.z, this.corner2.z].sort(
      (a, b) => a - b
    );
    this.raw = constr;
    this.inNodes = [];
...
export const solution = (input: string[]) => {
  ...
  for (let bi = 0; bi < bricks.length; bi++) {
    ...
    const careAbout = bricks.filter(
      (other) => brick.collidesXY(other) && brick != other
    );
    silly: while (true) {
  ...
  const removable = bricks.filter((brick) =>
    brick.outNodes.some((other) => other.inNodes.length == 1)
  );
